...
const FutureContinuousTense = () => {
  ...
  useEffect(() => {
    fetch("/sentences/sentences.json")
      .then((res) => res.json())
      .then((data) => {const futureContinuous = data.filter(sentence=> sentence.category ==="Future Continuous Tense");
       setFutureContinuousTense(futureContinuous)});
  }, []);
...
<div className="flex flex-wrap  bg-red-400">
        {futureContinuousTense?.map((sentence) => (
