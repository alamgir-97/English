...
const FuturePerfectTense = () => {
  ...
  useEffect(() => {
    fetch("/sentences/sentences.json")
      .then((res) => res.json())
      .then((data) => {const futurePerfect = data.filter(sentence=> sentence.category ==="Future Perfect Tense");
       setFuturePerfectTense(futurePerfect)});
  }, []);
...
<div className="flex flex-wrap  bg-red-400">
        {futurePerfectTense?.map((sentence) => (
