...
const FuturePerfectContinuousTense = () => {
  ...
  useEffect(() => {
    fetch("/sentences/sentences.json")
      .then((res) => res.json())
      .then((data) => {const futurePerfectContinuous = data.filter(sentence=> sentence.category ==="Future Perfect Continuous Tense");
       setFuturePerfectContinuousTense(futurePerfectContinuous)});
  }, []);
...
<div className="flex flex-wrap  bg-red-400">
        {futurePerfectContinuousTense?.map((sentence) => (
