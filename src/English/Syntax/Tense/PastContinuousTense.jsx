...
const PastContinuousTense = () => {
  ...
  useEffect(() => {
    fetch("/sentences/sentences.json")
      .then((res) => res.json())
      .then((data) => {const pastContinuous = data.filter(sentence=> sentence.category ==="Past Continuous Tense");
       setPastContinuousTense(pastContinuous)});
  }, []);
...
<div className="flex flex-wrap  bg-red-400">
        {pastContinuousTense?.map((sentence) => (
