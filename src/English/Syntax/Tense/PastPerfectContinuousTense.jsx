...
const PastPerfectContinuousTense = () => {
  ...
  useEffect(() => {
    fetch("/sentences/sentences.json")
      .then((res) => res.json())
      .then((data) => {const pastPerfectContinuous = data.filter(sentence=> sentence.category ==="Past Perfect Continuous Tense");
       setPastPerfectContinuousTense(pastPerfectContinuous)});
  }, []);
...
<div className="flex flex-wrap  bg-red-400">
        {pastPerfectContinuousTense?.map((sentence) => (
