...
const FutureIndefiniteTense = () => {
  const [futureIndefiniteTense, setFutureIndefiniteTense] = useState();

  //future Indefinite Tense
  useEffect(() => {
    fetch("/sentences/sentences.json")
      .then((res) => res.json())
      .then((data) => {const futureIndefinite = data.filter(sentence=> sentence.category ==="Future Indefinite Tense");
       setFutureIndefiniteTense(futureIndefinite)});
  }, []);
...
<div className="flex flex-wrap  bg-red-400">
        {futureIndefiniteTense?.map((sentence) => (
...
export default FutureIndefiniteTense;
