...
const Animals = () => {
  ...
  useEffect(() => {
    fetch("/words/words.json")
      .then((res) => res.json())
      .then((data) => {
        const animals = data.filter((item) => item.category === "animals/জীবজন্তু");
        setAnimals(animals);
      });
  }, []);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 bg-green-200 w-[70vw] h-[38vw] overflow-y-scroll">
        {animals?.map((word) => (
