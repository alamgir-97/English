...
const BodyParts = () => {
  ...
  useEffect(() => {
    fetch("/words/words.json")
      .then((res) => res.json())
      .then((data) => {
        const bodyParts = data.filter((item) => item.category === "body parts");
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 bg-green-200 w-[75vw] h-[38vw] overflow-y-scroll">
        {bodyParts?.map((word) => (
...
//   };
//   return (
//     <div className="border mx-5 my-3 p-4 bg-white w-[20vw] h-[33vw] rounded-xl hover:border-3">
